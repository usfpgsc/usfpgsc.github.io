...
The *outer product* is slightly different. The result is a matrix with pairwise products as entries. So the $j^{th}$ row is essentially $x_1 \mathbf{y}$. From this, it is clear I can use the outer product to find the ratio of those two integers in each row.  Shown below is a solution to part two:
